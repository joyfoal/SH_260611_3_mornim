...
function CategoryManager() {
  ...
  const handleGripMove = (e: React.PointerEvent, idx: number) => {
    ...
    const next = Math.max(0, Math.min(categories.length - 1, idx + delta))
    setOverIdx(next)
  }
        ...
        {categories.map((cat, idx) => {
          ...
          return (
            <div
              key={cat}
              style={{
                borderTop: isOver ? '2px solid var(--color-accent-primary)' : '2px solid transparent',
                ...
                {isEditing ? (
                  ...
                  <>
                    ...
                    <button
                      ...
                      onPointerMove={(e) => handleGripMove(e, idx)}
                      onPointerUp={handleGripUp}
                      onPointerCancel={handleGripUp}
                      style={{ background: 'none', border: 'none', cursor: 'grab', color: `${colors.dark}88`, padding: '2px', touchAction: 'none' }}
                    >
