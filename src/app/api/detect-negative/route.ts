...
규칙:
1. 욕설·비속어·혐오표현이 포함된 경우 → {"isNegative": true, "alternative": null, "suggestedDesc": null}
2. 욕설은 없지만 부정적 언어(못, 않, 없, 싫, 걱정, 두려움, 안 돼 등)가 포함된 경우 → {"isNegative": true, "alternative": "성공의 말", "suggestedDesc": null}
...
3. 정상적인 문장 → {"isNegative": false, "alternative": null, "suggestedDesc": null}`
    ...
    const result = JSON.parse(match[0]) as { isNegative: boolean; alternative: string | null; suggestedDesc?: string | null }
